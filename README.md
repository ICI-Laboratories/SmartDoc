...
* **Base de Datos Vectorial (Implícita)**: Vectores almacenados en archivos `.npz` y consultados eficientemente.
...
# Instalar dependencias del Document Processor
pip install -r document_processor/requirements.txt

# Instalar dependencias del LLM Service
...
Todas las peticiones a los microservicios deben pasar a través del **API Gateway**. Para identificarse, el cliente (el frontend) debe incluir un header HTTP:
  ...
  * **Header**: `X-User-ID`
  * **Valor**: Un nombre de usuario único (ej. `pedro-garcia`)
